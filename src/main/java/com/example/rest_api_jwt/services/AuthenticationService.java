...
import com.example.rest_api_jwt.pojos.AuthenticateRequest;
import com.example.rest_api_jwt.pojos.AuthenticationResponse;
import com.example.rest_api_jwt.pojos.RegisterRequest;
...
public class AuthenticationService {
    ...
    public ResponseEntity<?> register(RegisterRequest request) {
        ...
        if (!request.getEmail().matches("^(.+)@(.+)$"))
            return new ResponseEntity(new ApiResponse(false, "Email is not valid!", 400, null),
                    HttpStatus.BAD_REQUEST);
